...
- `agent/loop.ts` — `AgentLoop` class. Runs the LLM→tool→LLM cycle with abort support, pause/resume, hint injection, and progress tracking. Returns `LoopResult`. Unified mode: parallel tool execution, modal auto-dismiss, nudge→escalate→give-up for text-only responses. Workspace-scoped via `workspaceId` property — each workspace gets isolated state. Barrel-exported via `agent/index.ts`.
...
- `llm/client.ts` — `LLMClient`. Priority-based provider failover for the fast model tier: Cerebras (3000 TPS) → Groq (250K TPM) → OpenRouter (fallback). `ProviderPool` manages cooldowns (60s on 429) and immediate failover. Two model tiers: `MODEL_FAST_CEREBRAS` / `MODEL_FAST_GROQ` / `MODEL_FAST` (per-provider) and `MODEL_SMART` (MiniMax M2.5, always OpenRouter). `switchToSmart()` / `switchToFast()` for escalation. `llm/types.ts` defines `LLMMessage`, `CompletionRequest`, `CompletionResponse`, `ProviderConfig`. Barrel-exported via `llm/index.ts`.
