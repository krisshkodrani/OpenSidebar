...
interface FleetTelemetryDrainOptions {
  now?: number;
  random?: () => number;
  retryPolicy?: FleetTelemetryRetryPolicy;
}

const activeDrains = new WeakMap<FleetTelemetryStorageArea, Promise<FleetTelemetryDrainResult>>();

export function drainFleetTelemetryToTransport(
  storage: FleetTelemetryStorageArea,
  transport: FleetTelemetryTransport,
  options: FleetTelemetryDrainOptions = {},
): Promise<FleetTelemetryDrainResult> {
  const active = activeDrains.get(storage);
  if (active) return active;
  const drain = drainQueue(storage, transport, options).finally(() => {
    activeDrains.delete(storage);
  });
  activeDrains.set(storage, drain);
  return drain;
}
...
async function drainQueue(
  ...
  try {
    ...
    while (queue.length > 0) {
      if ((await loadFleetTelemetryConsent(storage)).status !== "enabled") {
        await clearLocalFleetTelemetry(storage);
        result.remaining = 0;
        return result;
      }
