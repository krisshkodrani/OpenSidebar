/**
 * ServiceNow adapter — page-scripted tool registrations.
 *
 * Registers the ServiceNow page tools that drive the platform through injected
 * MAIN-world scripts: knowledge-base search, list filter/sort/action, and
 * catalog-item inspect/configure. Relocated verbatim out of tools/index.ts
 * (RFC LP-16 Phase 4) so the generic tools file no longer carries ~4.5K lines
 * of ServiceNow domain logic. Split into two entry points, each invoked at its
 * original ordinal position in registerTools() so the LLM-facing definition
 * order is unchanged.
 *
 * Import-direction rule (see ./index.ts): this module must never import
 * "../index" or the "../tools" barrel — only ../helpers and concrete siblings.
 * The injected page scripts are self-contained serialized functions; they carry
 * no adapter imports, so moving the handlers here needs no injection changes.
 */
...
export function registerServiceNowListAndCatalogTools(
  toolRegistry: ToolRegistry,
): void {
  toolRegistry.register(
