...
describe("internal fleet telemetry uploader", () => {
  ...
  test("overlapping drains deliver a queued record only once", async () => {
    const storage = createFakeStorageArea();
    await saveFleetTelemetryConsent(storage, true, 100);
    await enqueueFleetTelemetry(storage, envelope(1), 101);
    let accept!: (response: Response) => void;
    const fetchImpl = vi.fn(() => new Promise<Response>((resolve) => { accept = resolve; }));
    const options = { endpoint: "https://telemetry.example.test/v1/telemetry", fetchImpl, now: 200 };
    const first = drainInternalFleetTelemetry(storage, options);
    await vi.waitFor(() => expect(fetchImpl).toHaveBeenCalledOnce());
    const second = drainInternalFleetTelemetry(storage, options);
  ...
  test("revoked consent stops the next queued upload", async () => {
    const storage = createFakeStorageArea();
    await saveFleetTelemetryConsent(storage, true, 100);
    await enqueueFleetTelemetry(storage, envelope(1), 101);
    await enqueueFleetTelemetry(storage, envelope(2), 102);
    const fetchImpl = vi.fn(async () => {
      await saveFleetTelemetryConsent(storage, false, 150);
      return new Response(null, { status: 202 });
    });
    await drainInternalFleetTelemetry(storage, {
      endpoint: "https://telemetry.example.test/v1/telemetry", fetchImpl, now: 200,
    });
    expect(fetchImpl).toHaveBeenCalledOnce();
    expect(await loadFleetTelemetryQueue(storage, 200)).toEqual([]);
  });
