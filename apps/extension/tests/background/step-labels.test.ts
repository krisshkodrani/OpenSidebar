...
describe("formatStepLabel", () => {
  test("escalate with reason shows truncated reason", () => {
    const label = formatStepLabel(ToolName.ESCALATE, {
      reason: "This riddle requires multi-step logical reasoning",
    });
  ...
  test("done returns Done", () => {
    expect(formatStepLabel(ToolName.DONE, { summary: "all done" })).toBe(
      "Done",
    );
  });

  test("click_element shows tag ID", () => {
    expect(formatStepLabel(ToolName.CLICK_ELEMENT, { id: 7 })).toBe(
      "Click [7]",
    );
  });

  test("navigate shows hostname", () => {
    const label = formatStepLabel(ToolName.NAVIGATE, {
  ...
  test("type_text truncates long text", () => {
    const label = formatStepLabel(ToolName.TYPE_TEXT, {
      id: 1,
      text: "a".repeat(50),
    });
  ...
  test("read_page", () => {
    expect(formatStepLabel(ToolName.READ_PAGE, {})).toBe("Scanning page");
  });

  test("hide_element shows tag ID", () => {
    expect(formatStepLabel(ToolName.HIDE_ELEMENT, { id: 12 })).toBe(
      "Hide [12]",
    );
  });

  test("read_element shows text by default", () => {
    expect(formatStepLabel(ToolName.READ_ELEMENT, { id: 5 })).toBe(
      "Read text of [5]",
    );
  });
  ...
  test("read_element with attribute", () => {
    expect(
      formatStepLabel(ToolName.READ_ELEMENT, { id: 3, attribute: "href" }),
    ).toBe('Read "href" of [3]');
  });

  test("execute_js shows code preview", () => {
    const label = formatStepLabel(ToolName.EXECUTE_JS, {
      code: "document.title",
    });
  ...
  test("upload_file shows tag ID", () => {
    expect(formatStepLabel(ToolName.UPLOAD_FILE, { id: 8 })).toBe(
      "Upload file to [8]",
    );
  });
  ...
  test("right_click shows tag ID", () => {
    expect(formatStepLabel(ToolName.RIGHT_CLICK, { id: 4 })).toBe(
      "Right-click [4]",
    );
  });
  ...
  test("set_checkbox shows tag ID and value", () => {
    expect(
      formatStepLabel(ToolName.SET_CHECKBOX, { id: 2, checked: true }),
    ).toBe("Check [2]");
    expect(
      formatStepLabel(ToolName.SET_CHECKBOX, { id: 2, checked: false }),
    ).toBe("Uncheck [2]");
  });

  test("download_file shows hostname", () => {
    const label = formatStepLabel(ToolName.DOWNLOAD_FILE, {
  ...
  test("inspect_hidden with pattern", () => {
    const label = formatStepLabel(ToolName.INSPECT_HIDDEN, {
      pattern: "secret",
    });
    expect(label).toBe('Checking hidden page state: "secret"');
  });

  test("inspect_hidden without pattern", () => {
    expect(formatStepLabel(ToolName.INSPECT_HIDDEN, {})).toBe(
  ...
  test("workflow inspectors use readable labels", () => {
    expect(
      formatStepLabel(ToolName.INSPECT_CHART, { pattern: "Critical" }),
    ).toBe('Inspect chart: "Critical"');
    expect(formatStepLabel(ToolName.INSPECT_TABLE, {})).toBe(
      "Inspect table data",
    );
    expect(formatStepLabel(ToolName.INSPECT_FILTER_STATE, {})).toBe(
      "Inspect filter state",
    );
    expect(formatStepLabel(ToolName.INSPECT_CATALOG_ITEM, {})).toBe(
      "Inspect catalog item",
    );
  });

  test("xray_page", () => {
    expect(formatStepLabel(ToolName.XRAY_PAGE, {})).toBe(
