...
- [ ] `pnpm exec tsx scripts/workarena-doctor.ts` reports ready.
- [ ] `corepack pnpm exec nx test extension -- agent.test.ts tools.test.ts` passes after the latest runtime changes.
- [ ] `corepack pnpm exec tsx scripts/workarena-validate-reports.ts` passes on existing reports.
...
- [ ] Run `corepack pnpm exec tsx scripts/workarena-validate-reports.ts`.
