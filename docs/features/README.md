# Features Documentation

Complete documentation for all implemented features in OpenSidebar.

## Core Features

### [Browser Automation](./browser-automation.md)

Navigate, read, click, type, and scroll across web pages using natural language commands.

### [Workspace Management](./workspace-management.md)

Automatic Chrome Tab Groups that organize related tabs and isolate activities.

### [Streaming UI](./streaming-ui.md)

Real-time text streaming and visual feedback for AI responses.

### [Security & Privacy](./security.md)

Security model, data protection, and privacy-preserving design.

## Feature Interaction

These features work together to provide a seamless AI browsing experience:

```
User Command
    ↓
Browser Automation (actions)
    ↓
Workspace Management (organization)
    ↓
Streaming UI (real-time feedback)
    ↓
Security & Privacy (protection throughout)
```

## Getting Started

1. **Install extension** and configure a supported provider API key
2. **Click extension icon** on any tab to create workspace
3. **Type natural language commands** to interact with web pages
4. **Watch real-time streaming** as AI executes actions

## User Guides

### Basic Usage

- "Navigate to google.com" - Opens new tab and goes to URL
- "Click the Search button" - Interacts with page elements
- "Fill out the shipping form" - Multi-step form automation
- "Compare prices across these tabs" - Cross-tab workflows

### Advanced Workflows

- Research projects with automatic tab organization
- Shopping comparisons across multiple sites
- Form filling with saved personal information
- Content extraction and note-taking

## Technical Details

For implementation details, see the [Architecture Documentation](../architecture/).

For development setup, see [Contributing Guide](../../CONTRIBUTING.md).
