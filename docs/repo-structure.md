...
- `pnpm run release:package`: build `dist/`, then write the release zip, SHA-256 checksum, notes, and manifest under `.artifacts/releases/`
- `pnpm run release:preflight`: release artifact/version/commit validation and native-smoke/publication-readiness warnings before tagging
