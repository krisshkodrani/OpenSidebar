...
| `identity`                 | Opens Chrome's protected OAuth redirect flow so the user can explicitly sign the extension into their OpenSidebar account with Cognito PKCE. It is used only when the user selects cloud sign-in.                                                                                                                                                                 |
...
The extension has two explicit inference modes. In local mode, provider requests
go directly from Chrome to the selected BYOK provider. In optional cloud mode,
the user signs into OpenSidebar, stores a KMS-encrypted provider credential, and
provider requests stream through OpenSidebar's non-retaining relay. Relay
...
| ----------------------------------------------------------------- | -------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Website content                                                   | **Yes**  | During an active task, page text, structure, URL, title, and optional screenshots are sent directly to the configured provider in local mode or processed transiently by the OpenSidebar relay before reaching that provider in cloud mode. Relay content is not retained.                                                                              |
| Web history                                                       | **Yes**  | Only when the user explicitly asks a history task; matching entries become task context sent directly to the provider or transiently through the cloud relay. There is no passive history collection or relay retention.                                                                                                                                |
| Authentication information                                        | **Yes**  | Local-mode provider keys remain in Chrome and go directly to the provider. In optional cloud mode, the user explicitly uploads a provider key for verification and KMS-envelope-encrypted storage; it is decrypted only in memory for a provider request and is never returned to clients. Opaque OpenSidebar device-session tokens are stored locally. |
| Personal communications                                           | **Yes**  | Task messages are sent directly to the configured provider in local mode or processed transiently by the non-retaining relay in cloud mode.                                                                                                                                                                                                             |
| Personally identifiable information                               | **Yes**  | Cloud sign-in stores the account email and Cognito subject plus device metadata. Optional profile facts relevant to a task may also be sent to the provider; sensitive profile items require explicit per-task consent.                                                                                                                                 |
