...
Delete `runPlanDeliberation()` and `shouldRunDeliberation()`. The initial `planner.decompose()` call produces the plan in one pass.

**Rationale from Rothman (Ch 4):** "The Planner makes ONE call." Plan refinement is warranted when plan quality is poor, but the planner prompt already produces well-structured plans. If plan quality needs improvement, the correct fix is improving the planner prompt (Gulli Ch 17 MASS: "optimize individual agents"), not adding iterative refinement calls.
...
**S1 — No deliberation:**
- Test that `buildNodes()` calls `planner.decompose()` exactly once
...
- **Plan quality without deliberation.** If the initial `planner.decompose()` produces poor plans, there's no refinement step. Mitigated by: (a) the planner prompt is already well-optimized, (b) node-level replanning via `expandNode()` still handles runtime failures, (c) if plan quality regresses measurably in evals, we can add back a single deliberation round (1 call, not 2).
