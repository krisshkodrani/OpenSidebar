...
chrome.runtime.onMessage.addListener((message: MemoryWorkerMessage, _sender, _sendResponse) => {
    ...
    (async () => {
        ...
        try {
            if (payload.action === "add") {
                ...
                const voyResults = voyResultsRaw.neighbors.map((n: any, i: number) => ({
                    id: n.id,
                ...
                const fetchEntry = (id: string) => {
                    const stmt = db!.prepare("SELECT content, category, source_url, created_at FROM memories WHERE id = ?");
                    stmt.bind([id]);
                    let entry: any = { id };
                    if (stmt.step()) {
                        const row = stmt.get();
                ...
                const fused = reciprocalRankFusion(voyResults, ftsResults, limit || 5, fetchEntry);
