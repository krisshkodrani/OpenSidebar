    ...
    return (
        <div className="flex items-center gap-2 text-xs">
            <div className="relative flex items-center justify-center w-2 h-2">
                <span className={clsx("w-2 h-2 rounded-full transition-colors duration-300", getStatusColor(status), isActive && "animate-pulse")} />
                {isActive && (
            ...
            <div className="flex flex-col leading-tight">
                <span className={clsx("font-medium transition-colors duration-300 uppercase tracking-wider text-[10px]",
                    status === AgentStatus.ERROR ? "text-red-500" : "text-warm-500 dark:text-warm-400"
                )}>
                    {status}
                </span>
                {detail && status !== AgentStatus.IDLE && (
                    <span className="text-[10px] text-warm-400 dark:text-warm-500 max-w-[120px] truncate transition-colors duration-300" title={detail}>
                        {detail}
                    </span>
                )}
            </div>
