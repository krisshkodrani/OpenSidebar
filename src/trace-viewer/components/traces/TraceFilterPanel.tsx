...
export default function TraceFilterPanel({ onFiltersChanged }: TraceFilterPanelProps) {
  ...
  if (filters.day !== "all") chips.push({ key: "day", label: `Day: ${filters.day}` });
  if (filters.from) chips.push({ key: "from", label: `From: ${filters.from}` });
  if (filters.to) chips.push({ key: "to", label: `To: ${filters.to}` });
  if (filters.domain) chips.push({ key: "domain", label: `Website: ${filters.domain}` });
  if (filters.sessionPrefix) chips.push({ key: "session", label: `Session: ${filters.sessionPrefix}` });
  ...
  const handleChipRemove = (key: string) => {
    ...
    if (key === "domain") { setFilter("domain", ""); setLocalDomain(""); }
    if (key === "session") { setFilter("sessionPrefix", ""); setLocalSession(""); }
    onFiltersChanged();
  };
  ...
  return (
    <div className="p-4 border-b border-trace-border shrink-0">
      <h2 className="text-[15px] font-bold text-trace-text mb-1">Trace Sessions</h2>
      <div className="text-[11px] text-trace-muted mb-3">Agent execution traces</div>
